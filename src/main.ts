...
const isRootDomain = hostname === 'resfactae.xyz'
const pageTitle = 'Res factae'

if (isRootDomain) {
  document.body.dataset.route = 'landing'
  document.title = pageTitle
  app.innerHTML = `
    <main class="landing" aria-label="Res factae">
      <h1 class="landing-title">
        <a class="landing-link" href="https://romeo.resfactae.xyz">Res factae</a>
      </h1>
    </main>
  `
} else {
  ...
  async function loadCatImageUrls() {
    if (!storage) {
      return loadCatImageUrlsFromPublicApi()
    }
    ...
    try {
      ...
      // Fall back to public API listing when SDK requests are blocked.
    }

    return loadCatImageUrlsFromPublicApi()
  }

  async function loadCatImageUrlsFromPublicApi() {
    ...
    endpoint.searchParams.set('prefix', 'cats/')
    ...
    const payload = (await response.json()) as {
      items?: Array<{ name?: string; downloadTokens?: string }>
    }

    const items = payload.items ?? []

    return items
      .filter((item) => typeof item.name === 'string' && imageExtension.test(item.name))
      .map((item) => {
        const encodedPath = encodeURIComponent(item.name as string)
        const token = item.downloadTokens?.split(',')[0]?.trim()
        const url = new URL(`https://firebasestorage.googleapis.com/v0/b/${bucket}/o/${encodedPath}`)
